...
Phase 5 までで S3 にテレメトリは溜まるようになったが、LLM 出力の **精度（事実誤認・汎用フレーズへの逃げ・季節整合）** を継続的に検証する仕組みは未整備。実データ（2026-04-26〜27 の 8 entry）を観察した結果、

- 「相模原市は…城下町です」「江戸時代から続く城跡」のような **事実誤認の疑い** がある記述
- 「春野菜」「桜が美しく咲き誇る」のような **他市町村でも通る汎用フレーズ** が散見
- 情緒・抒情に流れる文体（事実ベースで淡々が望ましい）
...
| 軸の構成 | **3 LLM 軸 + 1 機械軸** | 軸を分けて別 prompt にすることで迎合（Score Inflation）を抑える |
...
| # | 軸名 | 判定方式 | 内容 |
|---|---|---|---|
| 1 | 事実正確性 | Sonnet + Wikipedia RAG | Wikipedia 抜粋に根拠のない記述を引用列挙、5 点満点 |
| 2 | 具体性 | Sonnet 単独 | 「春野菜」「桜が美しい」など他市町村でも通る汎用フレーズを引用列挙、5 点満点 |
| 3 | 季節整合・文体 | Sonnet 単独 | 二十四節気と矛盾する記述、情緒・抒情・季節の挨拶（「淡紅色に染まり」「のんびりとした春の時間」など）を引用列挙、5 点満点 |
| 4 | 文字数 | コードで `length` 判定 | 120〜180 字（仕様）から外れていれば即 NG |

合格条件: **LLM 軸すべて 4 点以上 かつ 文字数判定 OK**。
...
- 1 entry あたり Judge 呼出: 3 軸 × 約 2,000 入力 tok + 200 出力 tok ≒ \$0.025
- Wikipedia API: 無料、Workers Cache でほぼヒット
- 月間想定: 100 entries × 平均 1.3 回呼出（再生成考慮） = 130 判定 ≒ **\$3.3 / 月（約 500 円）**
- レイテンシ: 1 回目合格時 3〜5 秒、再生成発生時 6〜10 秒
...
| 6.2 | Judge prompts（3 軸別、Few-shot 込み）| `workers/src/judge_prompts.js` + テスト |
| 6.3 | Judge 統合（並列呼出 + スコア集約 + 文字数判定）| `workers/src/judge.js` + テスト |
