...
## 11. Phase 7 / Plan F: Plan E 完成度向上 + 観測強化

Plan E（PR #30）が本番反映され、Sonnet 4.6 による 4 軸判定 + 再生成フィードバックループ + キャッシュ汚染防止 が動作することを 6.7 で確認した。Plan F は Plan E の効き目を最大化するための補完と、効果測定の仕組み強化を目的とする 3 サブプラン構成。

### 11.1 ロードマップ

- F-1（最高優先）: Plan E 補完 — 政令市の区への完全対応 + Haiku 文字数遵守率改善 + Haiku 知識限界対策
- F-2（中優先、実走データが溜まってから）: 観測強化 — Judge スコアと dwell_ms の相関分析 + 既存キャッシュの遡及評価・無効化スクリプト
- F-3（低優先、随時）: ユーザー機能拡張 — 解説の「再生成」ボタン / 道の駅近隣表示 / GPX エクスポート

### 11.2 F-1: Plan E 補完

#### 背景

6.7 の本番動作確認で 2 つの課題が顕在化：

- **海老名市 / 立夏のリクエストで Haiku が 119 字を生成 → judgeAll の文字数チェックで Sonnet 判定すらされず再生成 → 再生成も同様に文字数 NG → judge_passed=false で確定**。Sonnet 4 軸の判定機構を活かせていない
- **相模原市南区のリクエストでは accuracy=2 / specificity=3 / density=3 と低スコア。Haiku が「茶畑」「古淵駅中心」「江戸期の河岸」など事実根拠のない記述を生成し、再生成でも改善せず**

加えて、6.1 で持ち越した「政令市の区への Wikipedia 検索が `municipality` 単独形式（例:「南区」）だと曖昧さ回避ページに当たり、`{municipality} ({prefecture})` フォールバックも missing（Wikipedia 上の正式タイトルは「南区 (相模原市)」のように親市名カッコ付き）」も未解決。

#### 内訳

| 項目 | 内容 | 主な変更ファイル |
|---|---|---|
| F-1.1 政令市の区対応 | フロント `muni.js` に N03_003（親市名）も抽出・送信、Worker 側で `${区} (${親市})` 形式の Wikipedia title を構築 | `public/assets/muni.js` / `workers/src/wikipedia.js` / `workers/src/anthropic.js` parseDescribeRequest / `docs/spec.md` API 仕様 |
| F-1.2 文字数遵守率改善 | `workers/src/anthropic.js` の system prompt を強化（120〜180 字の中央値 150 字を目標、要素を絞ってでも下限を割らない指示）。実走 S3 集計で文字数 NG 率を観測しながら調整 | `workers/src/anthropic.js` |
| F-1.3 Haiku 知識限界対策 | 軸 1 の deductions を再生成プロンプトに渡しても Haiku が同じ嘘を繰り返す場合、Wikipedia 抜粋を Haiku の system / user prompt にも参考情報として渡すか検討（generator にも RAG）。実装するかは F-1.1 / F-1.2 後に再判定 | `workers/src/anthropic.js` / `describe_flow.js` |

#### 期待効果

- Sonnet 4 軸判定の到達率が上がり、軸別平均スコアが意味のある集計対象になる
- 政令市の区でも軸 1（事実正確性）が機能し、誤情報のフィルタが効く
- judge_passed=true の合格率が上がり、Anthropic API 課金（再生成ぶん）も削減

### 11.3 F-2: 観測強化

#### F-2.1 Judge スコアと dwell_ms の相関分析

S3 に Plan E entry と暗黙シグナル（dwell_ms / re_visited_count）が同時に溜まる。これらの相関を見ることで「Judge スコアが高い解説 = ユーザが長く読む解説」が成立しているか検証する。成立していなければ Judge 軸の設計自体を見直す材料になる。

実装は `docs/analysis/prompts.md` に分析プロンプトを追加 + `fetch_entries.sh` の集計に「judge_passed=true / false で dwell_ms 平均が有意差あるか」を追加する程度の小規模。

#### F-2.2 既存キャッシュの遡及評価・無効化スクリプト

Plan E 導入前にローカル localStorage にキャッシュされた古い解説（fail-open 時代に書き込まれた可能性のあるもの）を、フロント起動時に Sonnet で再評価して NG なら破棄するワンショット処理。

PoC スケール（利用者 1 人）では手動で localStorage クリアでも事足りるので、運用で必要になった時点で実装する持ち越し。

### 11.4 F-3: ユーザー機能拡張

#### F-3.1 解説の「再生成」ボタン

ユーザが「この解説いまいち」と感じたとき、明示的に再生成をリクエストできるボタン。`fetchDescription` の `force_regenerate=true` フラグで Workers がキャッシュを無視して新規生成。Plan E の judge_passed=true 合格出力も、ユーザの主観で再生成可能になる。

#### F-3.2 道の駅近隣表示（plan.md / todo.md 既存項目）

進行方向 ±60°、1200 駅データ。実装規模が大きい（GeoJSON 取得 + 方位計算 + UI 配置）ので独立 PR 推奨。

#### F-3.3 GPX エクスポート（既存項目）

軌跡データを GPX 形式でダウンロード。Strava 等への転送用途。実装規模は小さい。
...
- 境界振動対策（市町村境界で行ったり来たりする時の生成頻発防止）
- 本格アイコンデザイン
- 軌跡の localStorage トリム戦略
- `download_n03.sh` の最終 ls を動的に
