...
**2026-05-05 再考**: F-3.1（再生成ボタン）はユーザが解説の品質を判断する材料を画面で持たない（Judge スコアは ⚙️ デバッグオーバーレイ専用）ため、「ユーザの主観で再生成」シナリオが成立しない。一旦取り下げ。Judge NG 時の自動再生成（Plan E 6.4b）でカバーされている前提。
将来的に必要になるのは「Judge が NG にしたが文字数オーバーや fail-open で表示されている entry」を ⚙️ オーバーレイから個別に再生成できるデバッグ用機能。これは観測ニーズが出てから実装する。

- [ ] **F-3.1（取り下げ）解説の「再生成」ボタン（一般ユーザ向け）**: 上記理由で見送り
...
- [x] ブランチ `fix/prompt-overrefusal` push 完了（3 コミット）
- [x] PR #40 作成 + main マージ完了（merge commit `2d8e418`、2026-05-07）
- [x] 本番反映完了（`bash workers/deploy_production.sh`、Worker Version `480807fe-7dae-4d12-85a4-5e672eb7b43c`、2026-05-07）
- [ ] **観測（人間タスク）**: 1〜2 週間の実走 → `fetch_entries.sh` で軸別平均と合格率の推移確認、合格率 30〜50% 改善を Issue #35 完了条件として閉じる
- [ ] **副次タスク**: `workers/deploy_production.sh` テスト 1 を新スキーマ（`solar_term: "07"` 等）に修正、curl のヘッダ出力で APP_PASSWORD が画面に出る問題（`-sv` → `-si`）も併せて潰す
