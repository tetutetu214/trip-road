...
- A. 起動時に「現在地の市町村」を必ず描画する（cache あれば cache、無ければ API 呼出）。コスト増だが UX 改善
- B. 上記 F-3.1「再生成」ボタンに統合し、ユーザ任意で再取得できるようにする。コスト中立だが手動操作必須
- C. judge 不合格の解説も localStorage に保存し、起動時はとりあえずそれを出す（現行に近い、精度より UX 重視）。再アクセス時の体感は良いが過去の悪い解説が固定化する懸念
- D. 「合格してない市町村にいるときは『精度確保のため再取得をお試しください』など明示文言を出す」。実装小、UX は妥協

判断材料: API コスト（Haiku + Sonnet × 4軸 + Wikipedia）、自宅滞在時間の長さ（無音時間が長い）、判定の合格率傾向。
