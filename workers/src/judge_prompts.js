...
/**
 * 二十四節気の番号 → {name, period}。
 *
 * Judge プロンプトの「節気名 + 期間」表示と、軸 3（季節整合）の判定基準に使う。
 * period は人間可読の概算（実際の節気は太陽黄経で決まり毎年 ±1 日程度ずれる）。
 *
 * 既存の `anthropic.js` SOLAR_TERM_MAP（番号 → 名前）とは責務が異なるため
 * あえて重複させている：anthropic は generator 用の最小マップ、judge は
 * period も含む拡張メタ。両者をまとめると generator の system prompt が
 * 不必要に膨らむ。
 */
