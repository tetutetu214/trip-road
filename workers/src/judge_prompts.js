...
【観点】 地理・歴史・地形に関する事実誤認のみ評価する。

【この軸で評価する】
- Wikipedia 抜粋と直接矛盾する記述（重く減点）
- 地理常識として明らかな誤り（例: 内陸の市町村に「太平洋に面した港町」、山地に「平坦な臨海工業地帯」）

【この軸で評価しない（他軸の責任。ここで触れたら採点を誤る）】
- 表現の冗長さ・重複・簡潔さの欠如 → 軸 4（情報密度）の責任
- 固有名詞の薄さ・汎用フレーズの多さ → 軸 2（具体性）の責任
- 季節（節気）と矛盾する記述 → 軸 3（季節整合）の責任
- Wikipedia に記載がないだけ（直接矛盾なし） → 減点しない

「Wikipedia と表現が重なる」「年号が記載されていないため」「地理常識として説明不足」「冗長」「簡潔さに欠ける」のような理由で減点してはならない。減点根拠は「直接矛盾」または「地理常識として明らかな誤り」のみ。
...
例C（5点想定、Wikipedia 抜粋と内容が重なるが事実として正しい）:
解説:「箱根町は神奈川県西部、足柄下郡に位置する観光地。芦ノ湖や箱根温泉、箱根神社で知られる。」
→ 出力: {"deductions": [], "score": 5, "notes": "Wikipedia の記述と一致しており事実誤認なし。表現が冗長か簡潔かは軸 4 の評価で、accuracy では触れない"}

例D（5点想定、季節と矛盾するが accuracy 軸では減点しない）:
解説:「立夏のころ、横浜中華街では春節祭が開催される。」
→ 出力: {"deductions": [], "score": 5, "notes": "春節祭の時期不一致は季節整合（軸 3）の問題。地理・歴史・地形の事実は誤っていないため accuracy では減点しない"}
