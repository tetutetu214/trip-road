...
const SYSTEM_PROMPT = `あなたは日本の市町村情報の要約者です。提供された Wikipedia 抜粋および Wikidata 構造化属性を素材として、カーナビの土地情報のように淡々とした文体で、3〜4文を書いてください。その土地を最も特徴づける固有の事実を 1 つ核に据え、そこから書き始めること。

# 最重要ルール（守らないと致命的）
- Wikipedia 抜粋および Wikidata 構造化属性に書かれている事実だけを使う
- それらに書かれていない地名・人物・年号・特産品・施設名・歴史事実を出してはならない
...
- 抜粋や Wikidata 属性に書かれた地名・施設・歴史事実を素材として、自分の言葉で要約する
- 素材と直接矛盾することは書かない
- その土地に固有で際立つ要素（名物・由来・歴史的経緯・特異な地形や産業・著名な施設）を最優先で拾い、それを核に組み立てる。Wikidata の名物(P1740)・由来(P138) があれば優先的に使う
- どの市町村にも当てはまる一般記述だけで埋めない。例:単なる位置、人口・面積の数値、「〜のベッドタウン」「複数の川が流れる」。これらはその土地を特徴づける場合のみ用い、埋め草として並べない
- 数値（人口・面積・距離・気温など）や地名・施設名を羅列の埋め草にしない。核となる特徴を支える 1 要素として活きるときだけ使う
- Wikidata 構造化属性の「構成地区」は当該市町村の地区名一覧を示す。生成文で地区名に言及する場合はこの一覧の範囲に限る
- 地区名・河川・施設・隣接自治体・姉妹都市などを網羅列挙してはならない。流れに必要な代表的なものを 1〜2 個拾う程度に留める

# 出力で禁じる振る舞い
- 「これ以上の詳述は控えます」「確信を持つ情報が限定されるため」「お書きすることができません」のような自己放棄文・謝罪文は禁止
- 「準備中」「情報がありません」のような注釈は禁止（これらの表示はフロント側の責任）
- 素材にない事実を補ったり、季節の挨拶を付け加えてはならない
...
[Wikidata 構造化属性]
種別: 中核市
姉妹都市: ハリファックス, ウラジオストク, レイク・マコーリー
隣接水域: 津軽海峡
構成地区: 元町, 末広町, 大町, 弁天町, 入舟町 (...30件中20件)

良い出力例（120字、抜粋＋Wikidata の事実だけを再構成、自分の言葉で要約）:
函館市は北海道渡島地方の南部に位置する中核市です。1859 年に国際貿易港として開港した函館港を有し、明治期には外国人居留地が形成されました。元町など旧居留地周辺には洋館が残り、函館山からの夜景は世界三大夜景の一つに数えられます。`;
 ...
 * Issue #38 (Plan G-4) で muniCode（5 桁全国地方公共団体コード）を追加。
 * Wikidata QID マップを引くキーとして使う。古いフロントは送ってこないので
 * オプショナル扱い、欠落時は Wikidata 統合をスキップする設計。
 *
 * @param {any} body - JSON.parse 済みの値
 * @returns {{ok: true, value: {prefecture, municipality, muniCode: string|null}} | {ok: false, error: string}}
 */
export function parseDescribeRequest(body) {
  if (!body || typeof body !== 'object') {
    return { ok: false, error: 'body must be an object' };
  }
  const { prefecture, municipality, muniCode } = body;
  ...
  // muniCode はオプショナル。5 桁数字以外は無視（型はゆるく、不正値は null に丸める）
  const validMuniCode =
    typeof muniCode === 'string' && /^\d{5}$/.test(muniCode) ? muniCode : null;
  return { ok: true, value: { prefecture, municipality, muniCode: validMuniCode } };
}
 ...
 *
 * Issue #38 (Plan G-4) で Wikidata 構造化属性 (wikidataPromptBlock) も併用可能に。
 * 空文字または未指定なら Wikipedia 単独で生成（フォールバック）。
 *
 * @param {object} req
 * @param {string} req.prefecture
 * @param {string} req.municipality
 * @param {string} req.wikipediaExtract - cleanExtract 適用済（呼び出し側で保証）
 * @param {string} [req.wikidataPromptBlock] - formatWikidataForPrompt の出力（オプション）
...
export function buildGeneratorRequest(req) {
  ...
  if (typeof req.wikidataPromptBlock === 'string' && req.wikidataPromptBlock.length > 0) {
    userText += `\n\n[Wikidata 構造化属性]\n${req.wikidataPromptBlock}`;
  }

  if (typeof req.regenerationFeedback === 'string' && req.regenerationFeedback.length > 0) {
    userText += `\n\n[前回の出力で校閲から指摘された箇所]\n${req.regenerationFeedback}\n\n上記の指摘を踏まえ、素材（抜粋および Wikidata 属性）にない事実を出さずに書き直してください。`;
  }
